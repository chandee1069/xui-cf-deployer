...
- 配置 Cloudflare DNS、SSL、Origin Rules 或固定 Argo 隧道
...
- Argo 隧道配置：`/etc/x-ui/argo/cloudflared.json`
- Argo 隧道凭据：`/etc/x-ui/argo/tunnel_token.json`
...
curl -fsSL -o xui_cf_deployer.py https://raw.githubusercontent.com/chandee1069/xui-cf-deployer/main/xui_cf_deployer.py
...
- 安装节点（需已安装 3x-ui）
- 安装节点(Argo隧道)（域名直连 cloudflared，无需开放入站端口）
- 卸载
- 查看上次订阅
...
- 创建 Cloudflare Account-level 固定 Tunnel
- 自动生成 CNAME DNS 记录指向 Tunnel
- 设置 SSL 模式为 `full`
- 自动安装 `cloudflared` 并注册为 systemd 服务
- 流量通过加密隧道直达本地端口，**无需开放入站端口**
- 订阅链接格式与原模式一致
...
- 删除上次创建的 x-ui 入站配置
- 恢复 Cloudflare Origin Rules / DNS 路由到安装前状态
- 恢复 Cloudflare SSL 到安装前值
- 停止并删除 cloudflared 服务（Argo 隧道模式）
- 删除本地状态文件
...
- Argo 隧道模式 cloudflared 安装失败：手动运行 `curl -fsSL https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-amd64 -o /usr/local/bin/cloudflared && chmod +x /usr/local/bin/cloudflared`